 ...
 * @details 根据目标地址自动判断是内核函数还是用户函数，
 *          并使用合适的 CR3 进行 Hook。支持跨页检测、地址验证、详细日志。
 *
 * @param TargetAddress 要 Hook 的目标地址（支持 R0/R3 自动检测）
 * @param HookFunction Hook 处理函数地址
 * @param ProcessId 进程 ID（仅用于 R3 地址）
 *                  - 0: 对于 R3 地址使用调用者进程，对于 R0 地址忽略
 *                  - 非 0: 对于 R3 地址使用指定进程，对于 R0 地址忽略
 *
