...
#if DISABLE_HYPERDBG_HYPEREVADE == TRUE
VOID
...
TransparentHandleSystemCallHook(GUEST_REGS * Regs)
{
    ...
    PCHAR  CallingProcess = g_EvadeCallbacks.CommonGetProcessNameFromProcessControlBlock(PsGetCurrentProcess());
...
TransparentHandleNtQuerySystemInformationSyscall(GUEST_REGS * Regs)
{
    ...
    case SystemExtendedProcessInformation:
    {
        ...
        g_EvadeCallbacks.SyscallCallbackSetTrapFlagAfterSyscall(Regs,
...
TransparentGetObjectNameFromAttributesVirtualPointer(UINT64 virtPtr)
{
    ...
    //
    if (g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(virtPtr, &Buf, sizeof(OBJECT_ATTRIBUTES)))
    {
        ...
        //
        if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess((UINT64)Buf.ObjectName, &NameBuf, sizeof(UNICODE_STRING)))
        {
            SimpleHvLog("BadRead");
            return NULL;
        }
        ...
        {
            SimpleHvLogError("Error allocating ImageName memory buffer");
        ...
        //
        if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess((UINT64)NameBuf.Buffer, ObjectNameBuf, NameBuf.Length + sizeof(WCHAR)))
        {
            SimpleHvLog("BadRead");
...
TransparentHandleNtQueryAttributesFileSyscall(GUEST_REGS * Regs)
{
    ...
    //
    if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Regs->r10, sizeof(OBJECT_ATTRIBUTES)))
    {
        //
...
TransparentHandleNtOpenDirectoryObjectSyscall(GUEST_REGS * Regs)
{
    ...
    // Check if the pointer given as the 3rd argument to the system call with type POBJECT_ATTRIBUTES is valid
    //
    if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Regs->r8, sizeof(OBJECT_ATTRIBUTES)))
    {
        //
...
TransparentHandleNtOpenFileSyscall(GUEST_REGS * Regs)
{
    ...
    if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Regs->r8, sizeof(OBJECT_ATTRIBUTES)))
    {
        ...
        for (UINT16 j = 0; j < (sizeof(HV_FILES) / sizeof(HV_FILES[0])); j++)
        {
            ...
            {
                SimpleHvLog("A call to NtOpenFile systemcall for a hypervisor specific file was made");
...
TransparentHandleNtQueryValueKeySyscall(GUEST_REGS * Regs)
{
    ...
    //
    if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Regs->rdx, sizeof(UNICODE_STRING)))
    {
        ...
        //
        if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Regs->rdx, &NameUString, sizeof(UNICODE_STRING)))
            return;
        ...
        if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess((UINT64)NameUString.Buffer, NameBuf, NameUString.Length + sizeof(WCHAR)))
        {
            PlatformMemFreePool(NameBuf);
            return;
        }
        ...
        for (ULONG i = 0; i < (sizeof(TRANSPARENT_DETECTABLE_REGISTRY_KEYS) / sizeof(TRANSPARENT_DETECTABLE_REGISTRY_KEYS[0])); i++)
        {
            if (!wcscmp(KeyName, TRANSPARENT_DETECTABLE_REGISTRY_KEYS[i]))
            {
                ...
                //
                if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Regs->rsp + 0x28, sizeof(UINT64)))
                {
                    g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess((UINT64)(Regs->rsp + 0x28), &ContextParams.OptionalParam3, sizeof(ULONG));
                }
                else
                {
                    SimpleHvLog("Process 0x%llx on thread %llx executed NtQueryValueKey systemcall but reading the provided arguments from %RSP failed", HANDLE_TO_UINT32(PsGetCurrentProcessId()), HANDLE_TO_UINT32(PsGetCurrentThreadId()));
                ...
                //
                if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Regs->rsp + 0x30, sizeof(UINT64)))
                {
                    g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess((UINT64)(Regs->rsp + 0x30), &ContextParams.OptionalParam4, sizeof(UINT64));
                }
                else
                {
                    SimpleHvLog("Process 0x%llx on thread %llx executed NtQueryValueKey systemcall but reading the provided arguments from %RSP failed", HANDLE_TO_UINT32(PsGetCurrentProcessId()), HANDLE_TO_UINT32(PsGetCurrentThreadId()));
...
TransparentHandleNtEnumerateKeySyscall(GUEST_REGS * Regs)
{
    ...
    {
        SimpleHvLog("Process 0x%llx on thread %llx executed NtEnumerateKey systemcall but reading the provided arguments from %RSP failed", HANDLE_TO_UINT32(PsGetCurrentProcessId()), HANDLE_TO_UINT32(PsGetCurrentThreadId()));
        return;
    }
...
TransparentHandleModuleInformationQuery(PVOID Ptr, UINT64 VirtualAddress, UINT32 BufferSize)
{
    ...
    if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(VirtualAddress, Ptr, BufferSize))
    {
        return FALSE;
    }
    return TRUE;
...
TransparentHandleProcessInformationQuery(SYSCALL_CALLBACK_CONTEXT_PARAMS * Params)
{
    ...
    if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam2, &PrevStructBuf, sizeof(SYSTEM_PROCESS_INFORMATION)))
    {
        return FALSE;
    }
    ...
    if (PrevStructBuf.NextEntryOffset == 0 ||
        !g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam2 + ReadOffset, &CurStructBuf, sizeof(SYSTEM_PROCESS_INFORMATION)))
    {
        return FALSE;
    }
    ...
    do
    {
        ...
        if (CurStructBuf.ImageName.Length != 0)
        {
            ...
            if (StringBuf == NULL)
            {
                SimpleHvLog("Error allocating ImageName memory buffer");
            ...
            //
            if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess((UINT64)CurStructBuf.ImageName.Buffer, StringBuf, CurStructBuf.ImageName.Length + sizeof(WCHAR)))
            {
            ...
            for (UINT16 i = 0; i < (sizeof(HV_Processes) / sizeof(HV_Processes[0])); i++)
            {
                if (!_wcsnicmp(ImageName, HV_Processes[i], (CurStructBuf.ImageName.Length) / sizeof(WCHAR)))
                {
                    ...
                    //
                    if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess((UINT64)(Params->OptionalParam2 + WriteOffset), &PrevStructBuf, sizeof(SYSTEM_PROCESS_INFORMATION)))
                    {
                        SimpleHvLogError("Failed to modify memory buffer for the SystemProcessInformation query system call");
                    }
                    ...
                    if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess((UINT64)(Params->OptionalParam2 + WriteOffset + PrevOffset + BufOffset), StringBuf, CurStructBuf.ImageName.Length))
                    {
                        SimpleHvLogError("Failed to modify memory buffer for the SystemProcessInformation query system call");
                    }
        ...
        //
        if (!g_EvadeCallbacks.CheckAccessValidityAndSafety((UINT64)(Params->OptionalParam2 + ReadOffset), sizeof(SYSTEM_PROCESS_INFORMATION)))
        {
            return FALSE;
        }
        ...
        if (MatchFound)
        {
            ...
            if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess((UINT64)(Params->OptionalParam2 + WriteOffset + PrevOffset), &CurStructBuf, sizeof(SYSTEM_PROCESS_INFORMATION)))
            {
                return FALSE;
            }
        }
        ...
        //
        if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam2 + ReadOffset, &CurStructBuf, sizeof(SYSTEM_PROCESS_INFORMATION)))
        {
            return FALSE;
        }
...
TransparentHandleFirmwareInformationQuery(UINT64 Ptr, UINT32 BufMaxSize, UINT64 BufSizePtr)
{
    ...
    //
    if (!g_EvadeCallbacks.CheckAccessValidityAndSafety(BufSizePtr, sizeof(ULONG)) ||
        !g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(BufSizePtr, &BufSize, sizeof(ULONG)))
    {
        return 0;
    }
    ...
    {
        SimpleHvLog("The intercepted data buffer was too large for modification, in total 0x%x bytes", BufMaxSize);
        SimpleHvLog("The system call return value was set to STATUS_INVALID_INFO_CLASS, but this could be detected as hypervisor intervention");
    ...
    if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Ptr, Buf, BufSize))
    {
         ...
         StructBuf->ProviderSignature == 0x4649524D))
    {
        ...
        for (ULONG i = 0; i < (sizeof(HV_FIRM_NAMES) / sizeof(HV_FIRM_NAMES[0])); i++)
        {
            for (ULONG j = 0; j < StructBuf->TableBufferLength; j++)
            {
                ...
                if (MatchStart != 0)
                {
                    SimpleHvLog("Found Match for %s", HV_FIRM_NAMES[i]);
                    ...
                    if (BufSize - MatchedStringLen + NewSubstringSize > BufMaxSize)
                    {
                        ...
                        g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Ptr, Buf, BufMaxSize);

                        //
                        // Update the required buffer size for the next call
                        //
                        BufSize = (BufSize - oldLength) + NewStringSize;
                        g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(BufSizePtr, &BufSize, sizeof(ULONG));
                    ...
                    //
                    if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Ptr, Buf, BufSize))
                    {
                        SimpleHvLog("Error writing to user-mode buffer: %llx", Ptr);
                    }

                    if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(BufSizePtr, &BufSize, sizeof(ULONG)))
                    {
                        SimpleHvLog("Error writing to user-mode buffer: %llx", BufSizePtr);
                    }
                }
            }
        }
...
TransparentReplaceVendorStringFromBufferWChar(SYSCALL_CALLBACK_CONTEXT_PARAMS * Params, ULONG DataOffset, ULONG DataLenOffset)
{
    ...
    //
    if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam4, sizeof(ULONG)))
    {
        //
        // Read the size of the data that the kernel wrote to the buffer
        //
        ULONG BufSize = 0;
        if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam4, &BufSize, sizeof(ULONG)))
        {
        ...
        //
        if (Params->OptionalParam3 >= 0xC00 || !g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, (UINT32)Params->OptionalParam3))
        {
            goto ReturnWithError;
        }
        ...
        if (!Buf || !g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam2, Buf, Params->OptionalParam3))
        {
            goto ReturnWithError;
        }
        ...
        for (UINT16 i = 0; i < (sizeof(HV_REGKEYS) / sizeof(HV_REGKEYS[0])); i++)
        {
            ...
            while (MatchStart != 0)
            {
                ...
                if (BufSize - MatchedStringLen + tempSize > Params->OptionalParam3)
                {
                    ...
                    g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam2, Buf, Params->OptionalParam3);

                    //
                    // Update the required buffer size for the next call
                    //
                    BufSize = (tempSize - MatchedStringLen) + oldLength;
                    g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam4, &BufSize, sizeof(ULONG));
                ...
                *(PULONG)((PBYTE)Buf + DataLenOffset) = NewStringSize;
                ...
                // Write the changes back to the user buffers
                //
                if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam2, Buf, BufSize))
                {
                    goto ReturnWithError;
                }

                if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam4, &BufSize, sizeof(ULONG)))
                {
                    goto ReturnWithError;
                }
    ...
    //
ReturnWithError:
    SimpleHvLog("A call for to read a registry entry, which could contain hypervisor specific data, was intercepted but the mitigations failed");
    SimpleHvLog("The caller process received the results in this virtual address: %llx", Params->OptionalParam2);
...
TransparentCallbackHandleAfterNtQueryValueKeySyscall(SYSCALL_CALLBACK_CONTEXT_PARAMS * Params)
{
    ...
    default:
    {
        SimpleHvLog("NtQueryValueKey was called with KeyValueInformationClass 0x%x, a handler for which has not been implemented", Params->OptionalParam1);
        return 0;
    }
    }
...
TransparentCallbackHandleAfterNtEnumerateKeySyscall(SYSCALL_CALLBACK_CONTEXT_PARAMS * Params)
{
    ...
    default:
    {
        SimpleHvLog("NtEnumerateKey was called with KeyInformationClass 0x%x, a handler for which has not been implemented", Params->OptionalParam1);
        return 0;
    }
    }
...
TransparentCallbackHandleAfterNtQuerySystemInformationSyscall(GUEST_REGS * Regs, SYSCALL_CALLBACK_CONTEXT_PARAMS * Params)
{
    ...
    case SystemCodeIntegrityInformation:
    {
        ...
        //
        if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, (UINT32)Params->OptionalParam3))
        {
            ...
            //
            g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam2, &Temp, Params->OptionalParam3);

            //
            // Modify the data and write it back to the information buffer to be passed to user mode
            //
            Temp.CodeIntegrityOptions = 0x01;
            g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam2, &Temp, Params->OptionalParam3);
        }
        else
        {
            SimpleHvLog("A call for the NtQuerySystemInformation system call requesting SystemCodeIntegrityInformation structure was made, but the usermode buffer was not captured");
        }
    ...
    case SystemExtendedProcessInformation:
    {
        ...
        if (Params->OptionalParam2 != 0x0 &&
            ...
            g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, (UINT32)Params->OptionalParam3))
        {
            if (!TransparentHandleProcessInformationQuery(Params))
            {
                ...
                // SimpleHvLog("Error while modifying the buffer for data query 0x02x", Params->OptionalParam1);
            }
        }
        break;
    }
    ...
    case SystemModuleInformation:
    {
            ...
            g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, (UINT32)Params->OptionalParam3))
        {
            ...
            if (Buf == NULL)
            {
                SimpleHvLogError("Err, insufficient memory");
                break;
            }
            ...
            //
            if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam2, Buf, Params->OptionalParam3))
            {
                SimpleHvLog("Error reading memory buffer given by the usermode call");
            }
            else
            {
    ...
    case SystemKernelDebuggerInformation:
    {
        ...
        if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, (UINT32)Params->OptionalParam3))
        {
            ...
            //
            WORD Temp = 0x0100;
            g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam2, &Temp, 2);
        }
    }
    case SystemFirmwareTableInformation:
    {
        if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, (UINT32)Params->OptionalParam3))
        {
            ...
            if (RetVal == 0x0)
            {
                SimpleHvLog("A query for SystemFirmwareTableInformation was made, but the transparent mitigation failed");
            }
            else if (RetVal != 0x1)
            {
                SimpleHvLog("Changing to %llx", RetVal);
                Regs->rax = RetVal;
            }
        }
        else
        {
            SimpleHvLog("A query for SystemFirmwareTableInformation was made, but the user-mode buffer was not captured");
        }
        break;
    }
                                      ...
                                      SYSCALL_CALLBACK_CONTEXT_PARAMS * Params)
{
    ...
    else if (Context == g_SystemCallNumbersInformation.SysNtQueryAttributesFile)
    {
        ...
        // Check if the obtained buffer pointer is valid
        //
        if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam1, sizeof(FILE_BASIC_INFORMATION)))
        {
            FILE_BASIC_INFORMATION Buf = {0};
            //
            // Copy over the data from the output buffer pointer
            //
            if (!g_EvadeCallbacks.MemoryMapperReadMemorySafeOnTargetProcess(Params->OptionalParam1, &Buf, sizeof(FILE_BASIC_INFORMATION)))
            {
                SimpleHvLogError("Err, Virtual memory read failed");
            }
            else
            {
                //
                ...
                if (!g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam1, &Buf, sizeof(FILE_BASIC_INFORMATION)))
                {
                    SimpleHvLogError("Err, Virtual memory write failed");
                }
            }
        }
        else
        {
            SimpleHvLog("A call for the NtQueryAttributeFile system call for a marked file was made, but the output buffer was not captured");
        }
    }
    ...
    {
        SimpleHvLog("A NtOpenDirectoryObject system call was made for a known directory that reveals hypervisor presence. process: %x, thread: %x\n",
                ProcessId,
                ThreadId);
        SimpleHvLog("No action to mitigate this was made as a handler for NtOpenDirectoryObject has not been implemented");
    }
    //
    ...
    else if (Context == g_SystemCallNumbersInformation.SysNtQueryInformationProcess)
    {
        ...
        case 0x07:
        {
            if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, sizeof(DWORD_PTR)))
            {
                //
                ...
                g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam2, &NoDebugPort, sizeof(DWORD_PTR));
            }
            break;
        }
        case 0x1f:
        {
            ...
            g_EvadeCallbacks.MemoryMapperWriteMemorySafeOnTargetProcess(Params->OptionalParam2, &notDebugged, sizeof(ULONG));
            break;
        }
        case 0x1e:
        {
            if (g_EvadeCallbacks.CheckAccessValidityAndSafety(Params->OptionalParam2, (UINT32)Params->OptionalParam3))
            {
                SimpleHvLog("Process %llx called the NtQueryInformationProcess system call with the ProcessDebugObject class, no transparent mitigations were performed", ProcessId);
            }
            break;
        }
        default:
    ...
    else
    {
        ...
        //
        SimpleHvLog("Transparent callback  for an unimplemented system call handle with the trap flag for process: %x, thread: %x, context: %llx RAX: %llx (p1: %llx, p2: %llx, p3: %llx, p4: %llx) \n",
                ProcessId,
                ThreadId,
                Context,
                Regs->rax,
