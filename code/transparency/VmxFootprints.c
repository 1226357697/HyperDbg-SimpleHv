...
TransparentCheckAndModifyMsrRead(PGUEST_REGS Regs, UINT32 TargetMsr)
{
    ...
    // {
    //     SimpleHvLog("RDMSR attempts to write to a reserved MSR range. MSR: %x",
    //             TargetMsr);
    //
    //     g_EvadeCallbacks.EventInjectGeneralProtection();
...
TransparentCheckAndModifyMsrWrite(PGUEST_REGS Regs, UINT32 TargetMsr)
{
    ...
    //     //
    //
    //     SimpleHvLog("WRMSR attempts to write to a reserved MSR range. MSR: %x, rax: %llx, rdx: %llx",
    //             TargetMsr,
    //             Regs->rax,
    //             Regs->rdx);
    //
...
TransparentCheckAndTrapFlagAfterVmexit()
{
    ...
    //
    g_EvadeCallbacks.HvHandleTrapFlag();
}
