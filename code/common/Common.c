...
CommonIsGuestOnUsermode32Bit()
{
    ...
    {
        SimpleHvLogError("Err, unknown value for cs, cannot determine wow64 mode");
    }
...
{
    SimpleHvLogError("HyperDbg cannot recover from this error, please provide the following information through the Git issues");

    SimpleHvLog("Target RIP: %llx\n", VCpu->LastVmexitRip);
    ...
    {
        SimpleHvLog("%02X ", Instruction[i] & 0xffU);
    }
