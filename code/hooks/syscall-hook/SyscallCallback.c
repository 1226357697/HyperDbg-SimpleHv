...
SyscallCallbackInitialize()
{
    ...
    if (!g_SyscallCallbackStatus)
    {
        ...
        {
             SimpleHvLog("Error while inserting EPT page hook for Windows system call handler at address 0x%p+3", Msr.Flags);
...
SyscallCallbackUninitialize()
{
    if (g_SyscallCallbackStatus)
    {
        ...
        {
            SimpleHvLog("Error while removing the EPT hook from windows syscall handler at address 0x%p+3", Msr.Flags);
                                       ...
                                       SYSCALL_CALLBACK_CONTEXT_PARAMS * Params)
{
    ...
    //
    // SimpleHvLog("Syscall callback set trap flag for process: %x, thread: %x\n", ProcessId, ThreadId);
