...
ExecTrapTraverseThroughOsPageTables(PVMM_EPT_PAGE_TABLE EptTable, CR3_TYPE TargetCr3, CR3_TYPE KernelCr3)
{
    ...
    {
        // SimpleHvLog("Address of Cr3Va: %llx", Cr3Va);

        PPAGE_ENTRY Pml4e = (PAGE_ENTRY *)&Cr3Va[i];

        if (Pml4e->Fields.Present)
        {
            // SimpleHvLog("PML4[%d] = %llx", i, Pml4e->Fields.PageFrameNumber);
            ...
            if (PdptVa != NULL)
            {
                ...
                {
                    // SimpleHvLog("Address of PdptVa: %llx", PdptVa);

                    PPAGE_ENTRY Pdpte = (PAGE_ENTRY *)&PdptVa[j];

                    if (Pdpte->Fields.Present)
                    {
                        // SimpleHvLog("PML3[%d] = %llx", j, Pdpte->Fields.PageFrameNumber);
                        ...
                        if (PdVa != NULL)
                        {
                            ...
                            {
                                // SimpleHvLog("Address of PdVa: %llx", PdVa);
                                ...
                                {
                                    // SimpleHvLog("PML2[%d] = %llx", k, Pde->Fields.PageFrameNumber);
                                    ...
                                    if (PtVa != NULL)
                                    {
                                        ...
                                        {
                                            // SimpleHvLog("Address of PtVa: %llx", PtVa);

                                            // PPAGE_ENTRY Pt = &PtVa[l];

                                            /* if (Pt->Fields.Present)
                                            {
                                                // SimpleHvLog("PML1[%d] = %llx", l, Pt->Fields.PageFrameNumber);
                                            }*/
                                        }
                                    }
                                }
...
ExecTrapReadRamPhysicalRegions()
{
    ...
    do
    {
        ...
        // SimpleHvLog("RAM Range, from: %llx to %llx", Address.QuadPart, Address.QuadPart + Size);
...
ExecTrapInitialize()
{
    ...
    {
        SimpleHvLog("Your processor doesn't support Mode-Based Execution Controls (MBEC), which is a needed feature for this functionality :(\n"
...
ExecTrapHandleMoveToAdjustedTrapState(VIRTUAL_MACHINE_STATE * VCpu, DEBUGGER_EVENT_MODE_TYPE TargetMode)
{
    ...
    {
        SimpleHvLogError("Err, Invalid target mode for execution trap: %x", TargetMode);
    }
}
                                 ...
                                 VMX_EXIT_QUALIFICATION_EPT_VIOLATION * ViolationQualification)
{
    ...
    if (!ViolationQualification->EptExecutableForUserMode && ViolationQualification->ExecuteAccess)
    {
        ...
        // For test purposes
        //
        // SimpleHvLog("Reached to the user-mode of the process (0x%x) is executed address: %llx", PsGetCurrentProcessId(), VCpu->LastVmexitRip);
    ...
    else if (!ViolationQualification->EptExecutable && ViolationQualification->ExecuteAccess)
    {
        ...
        // For test purposes
        //
        // SimpleHvLog("Reached to the kernel-mode of the process (0x%x) is executed address: %llx", PsGetCurrentProcessId(), VCpu->LastVmexitRip);
