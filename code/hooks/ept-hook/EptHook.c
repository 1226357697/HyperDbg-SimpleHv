                      ...
                      _In_ CR3_TYPE                   ProcessCr3)
{
    ...
    {
        SimpleHvLog("EptHookCreateHookPage  PhysicalBaseAddress failed");
    ...
    {
        SimpleHvLog("EptHookCreateHookPage  PhysicalBaseAddress is bigger than 512 GB");
    ...
    *(BYTE *)TargetAddressInFakePageContent = 0xcc;
    ...
    for (size_t i = 0; i < ProcessorsCount; i++)
    {
        ...
        if (!EptSplitLargePage(g_GuestState[i].EptPageTable, TRUE, PhysicalBaseAddress))
        {
            ...
            SimpleHvLog("Err, could not split page for the address : 0x%llx", PhysicalBaseAddress);
...
ExAllocatePoolWithTagHook(
    ...
    ULONG     Tag)
{
    SimpleHvLog("ExAllocatePoolWithTag Called with : Tag = 0x%x, Number Of Bytes = 0x%x, Pool Type = 0x%x ",
            Tag,
            NumberOfBytes,
            PoolType);
                       ...
                       CR3_TYPE                ProcessCr3)
{
    ...
    if (!PhysicalBaseAddress)
    {
        SimpleHvLog("EptHookPerformPageHook Find PhysicalBaseAddress Failed");
        VmmCallbackSetLastError(DEBUGGER_ERROR_INVALID_ADDRESS);
        return FALSE;
    }
    ...
    if (PhysicalBaseAddress >= SIZE_512_GB)
    {
        SimpleHvLog("EptHookPerformPageHook  PhysicalBaseAddress OUT OF 512GB");
                   ...
                   BOOLEAN ApplyDirectlyFromVmxRoot)
{
    ...
    else
    {
                         ...
                         (UINT64)NULL64_ZERO) == STATUS_SUCCESS)
        {
			...
			SimpleHvLog("Error while applying hidden breakpoint hook from VMX Non-Root Mode");
        }
    }
...
EptHook(PVOID TargetAddress, UINT32 ProcessId)
{
    ...
    {
        SimpleHvLog(" EptHook Cant called from root");
        return FALSE;
    }
                         ...
                         PVOID *                 OutTrampoline)
{
    ...
    // Log offset
    //
    // SimpleHvLog("OffsetIntoPage: 0x%llx", OffsetIntoPage);
    //

    if ((OffsetIntoPage + 19) > PAGE_SIZE - 1)
    {
        SimpleHvLogError("Err, function extends past a page boundary");
        return FALSE;
    }
    ...
    // For logging purpose
    //
    // SimpleHvLog("Number of bytes of instruction mem: %x", SizeOfHookedInstructions);
    ...
    //
    //
    // SimpleHvLog("Trampoline: 0x%llx", Hook->Trampoline);
    // SimpleHvLog("HookFunction: 0x%llx", HookFunction);
    ...
    BOOLEAN                                        ApplyDirectlyFromVmxRoot)
{
    ...
    else
    {
        if (VmxGetCurrentLaunchState())
        {
                ...
                LayoutGetCr3ByProcessId(ProcessId).Flags) == STATUS_SUCCESS)
            {
                ...
                // Test log
                //
                // SimpleHvLog("Hook applied from VMX Root Mode");
                //
                ...
                {
                    SimpleHvLog("Err, unable to notify all cores to invalidate their TLB "
        ...
        else
        {
            if (EptHookPerformPageHookMonitorAndInlineHook(VCpu,
                ...
                PageHookMask) == TRUE)
            {
                SimpleHvLogWarning("Hook applied (VM has not launched)");
                return TRUE;
            }
        }
    }
    ...
    //
    SimpleHvLogWarning("Err, hook was not applied");
                        ...
                        BOOLEAN *                            IsExecViolation)
{
    ...
    {
        //
        // SimpleHvLog("Guest RIP : 0x%llx tries to read the page at : 0x%llx", GuestRip, ExactAccessedAddress);
        //
    ...
    {
        //
        // SimpleHvLog("Guest RIP : 0x%llx tries to write on the page at : 0x%llx", GuestRip, ExactAccessedAddress);
        //
    ...
    {
        //
        // SimpleHvLog("Guest RIP : 0x%llx tries to execute the page at : 0x%llx", GuestRip, ExactAccessedAddress);
        //
...
EptHookRemoveEntryAndFreePoolFromEptHook2sDetourList(UINT64 Address)
{
    ...
    LIST_FOR_EACH_LINK(g_EptHook2sDetourListHead, HIDDEN_HOOKS_DETOUR_DETAILS, OtherHooksList, CurrentHookedDetails)
    {
        if (CurrentHookedDetails->HookedFunctionAddress == (PVOID)Address)
        {
            ...
            {
                SimpleHvLogError("Err, something goes wrong, the pool not found in the list of previously allocated pools by pool manager");
            }
            return TRUE;
        }
    }
...
EptHook2GeneralDetourEventHandler(PGUEST_REGS Regs, PVOID CalledFrom)
{
    ...
    //
    // SimpleHvLog("Hidden Hooked function Called with : rcx = 0x%llx , rdx = 0x%llx , r8 = 0x%llx ,  r9 = 0x%llx",
    //        Regs->rcx,
    //        Regs->rdx,
    //        Regs->r8,
    //        Regs->r9);
...
/**
 * @brief 自动判断并安装 EPT Inline Hook (R0/R3 自适应)
 * @details 这个函数集成了以下功能：
 *          1. 地址规范性检查
 *          2. R0/R3 自动检测
 *          3. CR3 自动选择（调用者/指定进程/系统）
 *          4. 跨页边界检测
 *          5. 物理地址验证
 *          6. 详细的日志输出
 *
 * @param TargetAddress 要 Hook 的目标地址
 * @param HookFunction Hook 处理函数地址
 * @param ProcessId 进程 ID（仅用于 R3 地址）
 *
    ...
    UINT32                  ProcessId)
{
    ...
    EPT_HOOKS_ADDRESS_DETAILS_FOR_EPTHOOK2 HookDetail = { 0 };
    UINT32   PageHookMask = PAGE_ATTRIB_EXEC_HIDDEN_HOOK;
    ...
    if (!TargetAddress)
    {
        SimpleHvLogError("[EptHookAuto] Error: TargetAddress is NULL");
        VmmCallbackSetLastError(DEBUGGER_ERROR_INVALID_ADDRESS);
        return FALSE;
    }
    ...
    if (!HookFunction)
    {
        SimpleHvLogError("[EptHookAuto] Error: HookFunction is NULL");
        VmmCallbackSetLastError(DEBUGGER_ERROR_INVALID_ADDRESS);
        return FALSE;
    }
    ...
    if (!CheckAddressCanonicality((UINT64)TargetAddress, &IsKernelAddress))
    {
        ...
        // Usually occurs when address's high bits are not set correctly
        //
        SimpleHvLogError("[EptHookAuto] Error: Address 0x%llx is non-canonical", TargetAddress);
        SimpleHvLogError("  Canonical ranges:");
        SimpleHvLogError("    - User (R3): 0x0000000000000000 - 0x00007FFFFFFFFFFF");
        SimpleHvLogError("    - Kernel (R0): 0xFFFF800000000000 - 0xFFFFFFFFFFFFFFFF");
        VmmCallbackSetLastError(DEBUGGER_ERROR_INVALID_ADDRESS);
        return FALSE;
    }
    ...
    //
    // ========================================
    // 3. Select CR3 Based on Address Type
    // ========================================
    //
    if (IsKernelAddress)
    {
        //
        // R0 Hook (Kernel Address)
        //
        // Kernel addresses are mapped to the same physical address in all processes
        // So this hook will be effective for all processes (System-wide Hook)
        //
        ProcessCr3 = LayoutGetCurrentProcessCr3();

        SimpleHvLog("[EptHookAuto] Kernel Address Hook");
        SimpleHvLog("  Target Address : 0x%llx (R0 - Kernel Space)", TargetAddress);
        SimpleHvLog("  Hook Handler   : 0x%llx", HookFunction);
        SimpleHvLog("  Scope          : System-wide (all processes)");
        SimpleHvLog("  CR3            : 0x%llx (System CR3)", ProcessCr3.Flags);

        if (ProcessId != 0)
        {
            SimpleHvLogWarning("  Note: ProcessId (%d) is ignored for kernel hooks", ProcessId);
        }
    }
    else
    {
        //
        // R3 Hook (User Mode Address)
        //
        // User mode addresses may be mapped to different physical addresses in different processes
        // So we need to use the correct process's CR3
        //
        if (ProcessId == 0)
        {
            //
            // ProcessId = 0: Use caller's process
            //
            // Get the actual Guest CR3 from VMCS Guest State
            // This is the CR3 that Guest was using when VM-Exit occurred
            //
            ProcessCr3 = LayoutGetExactGuestProcessCr3();

            SimpleHvLog("[EptHookAuto] User Address Hook (Caller's Process)");
            SimpleHvLog("  Target Address : 0x%llx (R3 - User Space)", TargetAddress);
            SimpleHvLog("  Hook Handler   : 0x%llx", HookFunction);
            SimpleHvLog("  Scope          : Caller's process only");
            SimpleHvLog("  CR3            : 0x%llx (Guest CR3 from VMCS)", ProcessCr3.Flags);
        }
        else
        {
            //
            // ProcessId != 0: Use specified process
            //
            ProcessCr3 = LayoutGetCr3ByProcessId(ProcessId);

            if (ProcessCr3.Flags == 0)
            {
                SimpleHvLogError("[EptHookAuto] Error: Failed to get CR3 for ProcessId %d", ProcessId);
                SimpleHvLogError("  Possible reasons:");
                SimpleHvLogError("    - Process does not exist");
                SimpleHvLogError("    - Process has exited");
                SimpleHvLogError("    - Invalid ProcessId");
                VmmCallbackSetLastError(DEBUGGER_ERROR_INVALID_PROCESS_ID);
                return FALSE;
            }

            SimpleHvLog("[EptHookAuto] User Address Hook (Specified Process)");
            SimpleHvLog("  Target Address : 0x%llx (R3 - User Space)", TargetAddress);
            SimpleHvLog("  Hook Handler   : 0x%llx", HookFunction);
            SimpleHvLog("  Process ID     : %d", ProcessId);
            SimpleHvLog("  Scope          : Process %d only", ProcessId);
            SimpleHvLog("  CR3            : 0x%llx", ProcessCr3.Flags);
        }
    }
    ...
    // ========================================
    //
    PhysicalAddress = (SIZE_T)VirtualAddressToPhysicalAddressByProcessCr3(
    ...
    if (!PhysicalAddress)
    {
        SimpleHvLogError("[EptHookAuto] Error: Failed to translate virtual address to physical");
        SimpleHvLogError("  Virtual Address: 0x%llx", TargetAddress);
        SimpleHvLogError("  CR3            : 0x%llx", ProcessCr3.Flags);
        SimpleHvLogError("  Possible reasons:");
        SimpleHvLogError("    - Address is not mapped in target process");
        SimpleHvLogError("    - Invalid page table structure");
        SimpleHvLogError("    - Wrong CR3");
        VmmCallbackSetLastError(DEBUGGER_ERROR_INVALID_ADDRESS);
        return FALSE;
    }

    SimpleHvLog("  Physical Address: 0x%llx", PhysicalAddress);

    //
    // ========================================
    // 5. Physical Address Range Check (512 GB Limit)
    // ========================================
    //
    // EPT current implementation limitation: physical address must be less than 512 GB
    // This is due to the EPT page table structure limitations
    //
    if (PhysicalAddress >= SIZE_512_GB)
    {
        SimpleHvLogError("[EptHookAuto] Error: Physical address exceeds 512 GB limit");
        SimpleHvLogError("  Physical Address: 0x%llx", PhysicalAddress);
        SimpleHvLogError("  Maximum Allowed : 0x%llx (512 GB)", SIZE_512_GB);
        SimpleHvLogError("  This is a limitation of the current EPT implementation");
        VmmCallbackSetLastError(DEBUGGER_ERROR_CANNOT_PUT_EPT_HOOKS_ON_PHYSICAL_ADDRESS_ABOVE_512_GB);
        return FALSE;
    }
    ...
    //
    // ========================================
    // 6. Page Boundary Check
    // ========================================
    //
    // EPT Hook needs to write a 19-byte absolute jump instruction at target address
    // If the offset is too close to page boundary, the jump will cross page boundary
    // Current implementation does not support cross-page hooks
    //
    OffsetIntoPage = ADDRMASK_EPT_PML1_OFFSET((SIZE_T)TargetAddress);

    if ((OffsetIntoPage + 19) > (PAGE_SIZE - 1))
    {
        SimpleHvLogError("[EptHookAuto] Error: Target function crosses page boundary");
        SimpleHvLogError("  Target Address  : 0x%llx", TargetAddress);
        SimpleHvLogError("  Page Offset     : 0x%llx (%lld bytes)", OffsetIntoPage, OffsetIntoPage);
        SimpleHvLogError("  Required Space  : 19 bytes (absolute jump instruction)");
        SimpleHvLogError("  Available Space : %lld bytes", PAGE_SIZE - OffsetIntoPage);
        SimpleHvLogError("  Page Boundary   : 0x%llx", (UINT64)PAGE_ALIGN(TargetAddress) + PAGE_SIZE);
        SimpleHvLogError("");
        SimpleHvLogError("  Suggestions:");
        SimpleHvLogError("    1. Hook at function entry (usually aligned)");

        //
        // Provide alternative aligned address suggestion
        //
        PVOID AlignedAddress = (PVOID)((UINT64)TargetAddress & ~0xFULL);  // 16-byte alignment
        if (AlignedAddress != TargetAddress)
        {
            SIZE_T AlignedOffset = ADDRMASK_EPT_PML1_OFFSET((SIZE_T)AlignedAddress);
            if ((AlignedOffset + 19) <= (PAGE_SIZE - 1))
            {
                SimpleHvLogError("    2. Try 16-byte aligned address: 0x%llx", AlignedAddress);
            }
        }

        SimpleHvLogError("    3. Hook a different location in the function");

        VmmCallbackSetLastError(DEBUGGER_ERROR_FUNCTION_CROSSES_PAGE_BOUNDARY);
        return FALSE;
    }
    else
    {
        SimpleHvLog("  Page Offset    : 0x%llx (%lld bytes into page)", OffsetIntoPage, OffsetIntoPage);
        SimpleHvLog("  Available Space: %lld bytes (19 bytes required)", PAGE_SIZE - OffsetIntoPage);
        SimpleHvLog("  Page Check     : PASSED (no boundary crossing)");
    }

    //
    // ========================================
    // 7. Check if Page is Already Hooked
    // ========================================
    //
    // EPT Hook operates at page granularity
    // Currently does not support multiple Inline Hooks on the same physical page
    //
    ...
    while (&g_EptState->HookedPagesList != TempList->Flink)
    {
        ...
        {
            SimpleHvLogError("[EptHookAuto] Error: Physical page already hooked");
            SimpleHvLogError("  Physical Address: 0x%llx", PhysicalAddress);
            SimpleHvLogError("  Existing Hook   : 0x%llx (Virtual)", HookedEntry->VirtualAddress);
            SimpleHvLogError("  New Hook Request: 0x%llx (Virtual)", TargetAddress);
            SimpleHvLogError("");
            SimpleHvLogError("  Current implementation does not support multiple hooks on same page");
            SimpleHvLogError("  You need to unhook the existing hook first");
            VmmCallbackSetLastError(DEBUGGER_ERROR_EPT_MULTIPLE_HOOKS_IN_A_SINGLE_PAGE);
            return FALSE;
        }
    }
    ...
    //
    // ========================================
    // 8. Setup Hook Details Structure
    // ========================================
    //
    ...
    if (Result)
    {
        SimpleHvLog("[EptHookAuto] SUCCESS: Hook installed successfully!");
        SimpleHvLog("  Physical Page  : 0x%llx", PhysicalAddress);
        SimpleHvLog("  EPT Entry Modified");
        SimpleHvLog("  Trampoline Created");

        if (IsKernelAddress)
        {
            SimpleHvLog("  Impact: All processes calling this kernel function");
        }
        else
        {
            if (ProcessId == 0)
            {
                SimpleHvLog("  Impact: Caller's process only");
            }
            else
            {
                SimpleHvLog("  Impact: Process %d only", ProcessId);
            }
        }
    }
    else
    {
        SimpleHvLogError("[EptHookAuto] FAILED: Hook installation failed");
        SimpleHvLogError("  Check previous error messages for details");
    }
