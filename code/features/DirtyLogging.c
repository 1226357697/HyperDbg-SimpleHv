...
DirtyLoggingInitialize()
{
    ...
    {
        SimpleHvLogWarning("err, dirty logging mechanism is not initialized as the processor doesn't support PML");
        return FALSE;
    }
...
DirtyLoggingEnable(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    // SimpleHvLog("PML Buffer Address = %llx", PmlPhysAddr);
 ...
 */
VOID
DirtyLoggingHandlePageModificationSimpleHvLog(VIRTUAL_MACHINE_STATE * VCpu)
{
    //
    ...
    {
        SimpleHvLog("Address : %llx", VCpu->PmlBufferAddress[i]);
    }
}
...
DirtyLoggingHandleVmexits(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    // Test log
    //
    SimpleHvLog("Dirty Logging VM-exit");
    // DirtyLoggingHandlePageModificationSimpleHvLog(VCpu);
