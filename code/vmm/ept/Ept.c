...
EptCheckFeatures(VOID)
{
    ...
    {
        SimpleHvLog("The processor doesn't report advanced VM-exit information for EPT violations");
    }

    if (!VpidRegister.ExecuteOnlyPages)
    {
        g_CompatibilityCheck.ExecuteOnlySupport = FALSE;
        SimpleHvLog("The processor doesn't support execute-only pages, execute hooks won't work as they're on this feature in our design");
    }
    else
    {
    ...
    {
        SimpleHvLogError("Err, MTRR dynamic ranges are not supported");
        return FALSE;
    }

    SimpleHvLog("All EPT features are present");
...
EptGetMemoryType(SIZE_T PageFrameNumber, BOOLEAN IsLargePage)
{
    ...
    for (CurrentMtrrRange = 0; CurrentMtrrRange < g_EptState->NumberOfEnabledMemoryRanges; CurrentMtrrRange++)
    {
        ...
        {
            // SimpleHvLog("0x%X> Range=%llX -> %llX | Begin=%llX End=%llX", PageFrameNumber, AddressOfPage, AddressOfPage + SIZE_2_MB - 1, g_EptState->MemoryRanges[CurrentMtrrRange].PhysicalBaseAddress, g_EptState->MemoryRanges[CurrentMtrrRange].PhysicalEndAddress);
...
EptBuildMtrrMap(VOID)
{
    ...
    for (CurrentRegister = 0; CurrentRegister < MTRRCap.VariableRangeCount; CurrentRegister++)
    {
        ...
        if (CurrentPhysMask.Valid)
        {
            ...
            SimpleHvLog("MTRR Range: Base=0x%llx End=0x%llx Type=0x%x", Descriptor->PhysicalBaseAddress, Descriptor->PhysicalEndAddress, Descriptor->MemoryType);
        }
    }

    SimpleHvLog("Total MTRR ranges committed: 0x%x", g_EptState->NumberOfEnabledMemoryRanges);
                  ...
                  SIZE_T              PhysicalAddress)
{
    ...
    if (!TargetEntry)
    {
        SimpleHvLogError("Err, an invalid physical address passed");
        return FALSE;
    }
    ...
    if (!NewSplit)
    {
        SimpleHvLogError("Err, failed to allocate dynamic split memory");
        return FALSE;
    }
...
EptAllocateAndCreateIdentityPageTable(VOID)
{
    ...
    if (PageTable == NULL)
    {
        SimpleHvLogError("Err, failed to allocate memory for PageTable");
        return NULL;
    }
...
EptLogicalProcessorInitialize(VOID)
{
    ...
    for (size_t i = 0; i < ProcessorsCount; i++)
    {
        ...
        if (!PageTable)
        {
            ...
            SimpleHvLogError("Err, unable to allocate memory for EPT");
            return FALSE;
        }
...
EptHandleEptViolation(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    SimpleHvLogError("Err, unexpected EPT violation at RIP: %llx", VCpu->LastVmexitRip);
    DbgBreakPoint();
    //
...
EptHandleMisconfiguration(VOID)
{
    ...
    SimpleHvLog("EPT Misconfiguration!");

    SimpleHvLogError("Err, a field in the EPT paging structure was invalid, faulting guest address : 0x%llx",
             GuestPhysicalAddr);
                           ...
                           INVEPT_TYPE             InvalidationType)
{
    ...
    {
        SimpleHvLogError("Err, invalid invalidation parameter");
    }
}
