...
VmxVmexitHandler(_Inout_ PGUEST_REGS GuestRegs)
{
    ...
    // Debugging purpose
    //
    // SimpleHvLog("VM_EXIT_REASON : 0x%x", ExitReason);
    // SimpleHvLog("VMCS_EXIT_QUALIFICATION : 0x%llx", VCpu->ExitQualification);
    //
    ...
    case VMX_EXIT_REASON_SMI:
    {
        ...
        //
        SimpleHvLog("VM-exit reason SMM %llx | qual: %llx", ExitReason, VCpu->ExitQualification);
    ...
    case VMX_EXIT_REASON_EPT_VIOLATION:
    {
        ...
        {
            SimpleHvLogError("Err, there were errors in handling EPT violation");
        }
    ...
    default:
    {
        ...
        // Not handled vm-exit
        //
        SimpleHvLogError("Err, unknown vmexit, reason : 0x%llx", ExitReason);
