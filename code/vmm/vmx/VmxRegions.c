...
VmxAllocateVmxonRegion(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    if (VmxonRegion == NULL)
    {
        SimpleHvLogError("Err, couldn't allocate buffer for VMXON region");
        return FALSE;
    }
    ...
    SimpleHvLog("VMXON Region Address : %llx", AlignedVmxonRegion);
    ...
    SimpleHvLog("VMXON Region Physical Address : %llx", AlignedVmxonRegionPhysicalAddr);

    //
    // get IA32_VMX_BASIC_MSR RevisionId
    //
    VmxBasicMsr.AsUInt = __readmsr(IA32_VMX_BASIC);
    SimpleHvLog("Revision Identifier (IA32_VMX_BASIC - MSR 0x480) : 0x%x", VmxBasicMsr.VmcsRevisionId);
    ...
    *(UINT64 *)AlignedVmxonRegion = VmxBasicMsr.VmcsRevisionId;
    ...
    if (VmxonStatus)
    {
        SimpleHvLogError("Err, executing vmxon instruction failed with status : %d", VmxonStatus);
        return FALSE;
    }
...
VmxAllocateVmcsRegion(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    if (VmcsRegion == NULL)
    {
        SimpleHvLogError("Err, couldn't allocate Buffer for VMCS region");
        return FALSE;
    }
    ...
    SimpleHvLog("VMCS region address : %llx", AlignedVmcsRegion);

    AlignedVmcsRegionPhysicalAddr = (UINT64)((ULONG_PTR)(VmcsPhysicalAddr + ALIGNMENT_PAGE_SIZE - 1) & ~(ALIGNMENT_PAGE_SIZE - 1));
    SimpleHvLog("VMCS region physical address : %llx", AlignedVmcsRegionPhysicalAddr);
...
VmxAllocateVmmStack(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating vmm stack");
        return FALSE;
    }

    SimpleHvLog("VMM Stack for logical processor : 0x%llx", VCpu->VmmStack);
...
VmxAllocateMsrBitmap(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating MSR Bitmaps");
        return FALSE;
    }

    VCpu->MsrBitmapPhysicalAddress = VirtualAddressToPhysicalAddress((PVOID)VCpu->MsrBitmapVirtualAddress);

    SimpleHvLog("MSR Bitmap virtual address  : 0x%llx", VCpu->MsrBitmapVirtualAddress);
    SimpleHvLog("MSR Bitmap physical address : 0x%llx", VCpu->MsrBitmapPhysicalAddress);
...
VmxAllocateIoBitmaps(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating I/O Bitmaps A");
        return FALSE;
    }

    VCpu->IoBitmapPhysicalAddressA = VirtualAddressToPhysicalAddress((PVOID)VCpu->IoBitmapVirtualAddressA);

    SimpleHvLog("I/O Bitmap A Virtual Address  : 0x%llx", VCpu->IoBitmapVirtualAddressA);
    SimpleHvLog("I/O Bitmap A Physical Address : 0x%llx", VCpu->IoBitmapPhysicalAddressA);
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating I/O Bitmaps B");
        return FALSE;
    }

    VCpu->IoBitmapPhysicalAddressB = VirtualAddressToPhysicalAddress((PVOID)VCpu->IoBitmapVirtualAddressB);

    SimpleHvLog("I/O Bitmap B virtual address  : 0x%llx", VCpu->IoBitmapVirtualAddressB);
    SimpleHvLog("I/O Bitmap B physical address : 0x%llx", VCpu->IoBitmapPhysicalAddressB);
...
VmxAllocateHostIdt(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating host IDT");
        return FALSE;
    }

    SimpleHvLog("Host IDT virtual address : 0x%llx", VCpu->HostIdt);
...
VmxAllocateHostGdt(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating host GDT");
        return FALSE;
    }

    SimpleHvLog("Host GDT virtual address : 0x%llx", VCpu->HostGdt);
...
VmxAllocateHostTss(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating host TSS");
        return FALSE;
    }

    SimpleHvLog("Host TSS virtual address : 0x%llx", VCpu->HostTss);
...
VmxAllocateHostInterruptStack(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    {
        SimpleHvLogError("Err, insufficient memory in allocating host interrupt stack");
        return FALSE;
    }

    SimpleHvLog("Host interrupt stack virtual address : 0x%llx", VCpu->HostInterruptStack);
