...
IdtEmulationPrepareHostIdt(_Inout_ VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    for (size_t i = 0; i < HOST_IDT_DESCRIPTOR_COUNT; i++)
    {
        ...
        // SimpleHvLog("IDT Entry [%d] at: %llx", i, Offset);
...
IdtEmulationhandleHostInterrupt(_Inout_ INTERRUPT_TRAP_FRAME * IntrTrapFrame)
{
    ...
    case EXCEPTION_VECTOR_PAGE_FAULT:
        ...
        SimpleHvLog("Page-fault received, rip: %llx, rsp: %llx, error: %llx, CR2: %llx",
                IntrTrapFrame->rip,
                IntrTrapFrame->rsp,
                IntrTrapFrame->error,
                PageFaultCr2);
    ...
    default:
        ...
        // host exceptions
        //
        SimpleHvLog("Host exception, rip: %llx, rsp: %llx, error: %llx, vector: %x",
                IntrTrapFrame->rip,
                IntrTrapFrame->rsp,
                IntrTrapFrame->error,
                             ...
                             _In_ VMEXIT_INTERRUPT_INFORMATION InterruptExit)
{
    ...
    // SimpleHvLog("#PF Fault = %016llx, Page Fault Code = 0x%x | %s%s%s%s",
    //         PageFaultAddress,
                                    ...
                                    _In_ VMEXIT_INTERRUPT_INFORMATION InterruptExit)
{
    ...
    else
    {
        ...
        SimpleHvLogError("Err, why we are here? it's a vm-exit due to the external"
