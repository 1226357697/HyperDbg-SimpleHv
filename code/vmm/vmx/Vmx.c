...
VmxCheckVmxSupport()
{
    ...
    {
        SimpleHvLogError("Err, you should enable vt-x from BIOS");
        return FALSE;
    }
...
VmxPerformVirtualizationOnAllCores()
{
    ...
    {
        SimpleHvLogError("Err, VMX is not supported in this machine");
        return FALSE;
    }
    ...
    if (!EptCheckFeatures())
    {
        SimpleHvLogError("Err, your processor doesn't support all EPT features");
        return FALSE;
    }
    else
    {
        ...
        //
        SimpleHvLog("Your processor supports all EPT features");
        ...
        if (!EptBuildMtrrMap())
        {
            SimpleHvLogError("Err, could not build MTRR memory map");
            return FALSE;
        }

        SimpleHvLog("MTRR memory map built successfully");
    }
    ...
    {
        SimpleHvLogError("Err, could not initialize pool manager");
        return FALSE;
    }
...
VmxPerformVirtualizationOnSpecificCore()
{
    ...
    SimpleHvLog("Allocating vmx regions for logical core %d", CurrentCore);
    ...
    SimpleHvLog("VMX-Operation enabled successfully");

    if (!VmxAllocateVmxonRegion(VCpu))
    {
        SimpleHvLogError("Err, allocating memory for vmxon region was not successful");
        return FALSE;
    }
    if (!VmxAllocateVmcsRegion(VCpu))
    {
        SimpleHvLogError("Err, allocating memory for vmcs region was not successful");
        return FALSE;
    }
...
VmxVirtualizeCurrentSystem(PVOID GuestStack)
{
    ...
    SimpleHvLog("Virtualizing current system (logical core : 0x%x)", CurrentCore);
    ...
    {
        SimpleHvLogError("Err, failed to clear vmcs");
        return FALSE;
    }
    ...
    if (!VmxLoadVmcs(VCpu))
    {
        SimpleHvLogError("Err, failed to load vmcs");
        return FALSE;
    }

    SimpleHvLog("Setting up VMCS for current logical core");

    VmxSetupVmcs(VCpu, GuestStack);

    SimpleHvLog("Executing VMLAUNCH on logical core %d", CurrentCore);
    ...
    SimpleHvLogError("Err, unable to execute VMLAUNCH, status : 0x%llx", ErrorCode);

    //
    // Then Execute Vmxoff
    //
    __vmx_off();
    SimpleHvLogError("Err, VMXOFF Executed Successfully but it was because of an error");
...
VmxTerminate()
{
    ...
    {
        SimpleHvLog("VMX terminated on logical core %d\n", CurrentCore);
...
VmxVmptrst()
{
    ...
    SimpleHvLog("VMPTRST result : %llx", VmcsPhysicalAddr);
}
...
VmxClearVmcsState(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    SimpleHvLog("VMCS VMCLEAR status : 0x%x", VmclearStatus);
    ...
    if (VmclearStatus)
    {
        ...
        //
        SimpleHvLog("VMCS failed to clear, status : 0x%x", VmclearStatus);
        __vmx_off();
        return FALSE;
    }
    return TRUE;
...
VmxLoadVmcs(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    if (VmptrldStatus)
    {
        SimpleHvLog("VMCS failed to load, status : 0x%x", VmptrldStatus);
        return FALSE;
    }
    return TRUE;
}
...
VmxSetupVmcs(VIRTUAL_MACHINE_STATE * VCpu, PVOID GuestStack)
{
    ...
    SimpleHvLog("CPU Based VM Exec Controls (Based on %s) : 0x%x",
                 VmxBasicMsr.VmxControls ? "IA32_VMX_TRUE_PROCBASED_CTLS" : "IA32_VMX_PROCBASED_CTLS",
    ...
    SimpleHvLog("Secondary Proc Based VM Exec Controls (IA32_VMX_PROCBASED_CTLS2) : 0x%x", SecondaryProcBasedVmExecControls);
...
VmxVmresume()
{
    ...
    SimpleHvLogError("Err, in executing VMRESUME, status : 0x%llx, last VM-exit reason: 0x%x",
             ErrorCode,
...
VmxPerformTermination()
{
    ...
    SimpleHvLog("VMX operation turned off successfully");
}
